...
public final class TnetStatusCode {
    ...
    public static final int getStatusCode(int i) {
        if (i <= EASY_DEFAULT_ERROR) {
            return i;
        }
        return i <= EASY_REASON_SLIGHTSSL_ERROR ? EASY_REASON_SLIGHTSSL_ERROR : i <= EASY_REASON_HANDSHAKE_ERROR ? EASY_REASON_HANDSHAKE_ERROR : i <= EASY_REASON_CONN_ERROR ? EASY_REASON_CONN_ERROR : i <= EASY_REASON_WRITE_ERROR ? EASY_REASON_WRITE_ERROR : i <= EASY_REASON_READ_ERROR ? EASY_REASON_READ_ERROR : i;
    }
    ...
    public static final int getErrno(int i) {
        ...
        if (i <= EASY_REASON_SLIGHTSSL_ERROR) {
            return -3500 - i;
        }
        if (i <= EASY_REASON_HANDSHAKE_ERROR) {
            return -3000 - i;
        }
        if (i <= EASY_REASON_CONN_ERROR) {
            return -2500 - i;
        }
        if (i <= EASY_REASON_WRITE_ERROR) {
            return -2300 - i;
        }
        if (i <= EASY_REASON_READ_ERROR) {
            return -2100 - i;
        }
        return 0;
    }
