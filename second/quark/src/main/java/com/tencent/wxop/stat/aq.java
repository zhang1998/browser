...
final class aq extends SQLiteOpenHelper {
    ...
    public aq(Context context, String str) {
        ...
        if (i.b()) {
            as.d.a("SQLiteOpenHelper " + this.a);
        }
    }
        ...
        try {
            ...
            try {
                ...
                while (query.moveToNext()) {
                    arrayList.add(new ar(query.getLong(0), query.getString(1), query.getInt(2), query.getInt(3)));
                }
                ContentValues contentValues = new ContentValues();
                for (ar arVar : arrayList) {
                    contentValues.put("content", s.b(arVar.b));
                    sQLiteDatabase.update("events", contentValues, "event_id=?", new String[]{Long.toString(arVar.a)});
                }
                if (query != null) {
                    query.close();
                }
    ...
    public final void onUpgrade(SQLiteDatabase sQLiteDatabase, int i, int i2) {
        as.d.f("upgrade DB from oldVersion " + i + " to newVersion " + i2);
        if (i == 1) {
            sQLiteDatabase.execSQL("create table if not exists keyvalues(key TEXT PRIMARY KEY NOT NULL, value TEXT)");
            a(sQLiteDatabase);
            b(sQLiteDatabase);
